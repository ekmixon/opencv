...
def getParameters(i, tokens):
    ...
    params = []
    while not (i >= len(tokens) or tokens[i] == ')'):
...
def generateStaticDefinitions(fns):
    ...
    for fn in fns:
        ...
        print(
            (
                commentStr
                + f"CL_RUNTIME_EXPORT {' '.join(fn['modifiers'] + ' ') if len(fn['modifiers']) > 0 else ''}{' '.join(fn['ret'])} ({' '.join(fn['calling'])} *{fn['name']}_pfn)({', '.join(fn['params'])}) = {fn['name']};"
            )
        )
...
def generateRemapDynamic(fns):
    ...
    for fn in fns:
        ...
        commentStr = '' if 'enabled' in fn else '//'
        print(commentStr + f"#define {fn['name']} {fn['name']}_pfn")
...
def generateFnDeclaration(fns):
    ...
    for fn in fns:
        ...
        print(
            (
                commentStr
                + f"extern CL_RUNTIME_EXPORT {' '.join(fn['modifiers'])} {' '.join(fn['ret'])} ({' '.join(fn['calling'])} *{fn['name']})({', '.join(fn['params'] if 'params_full' not in fn else fn['params_full'])});"
            )
        )
...
def generateInlineWrappers(fns):
    ...
    for fn in fns:
        commentStr = '' if 'enabled' in fn else '//'
        print(f"#undef {fn['name']}")
        print(commentStr + f"#define {fn['name']} {fn['name']}_fn")
        params = []
        call_params = []
        for i in range(len(fn['params'])):
            t = fn['params'][i]
