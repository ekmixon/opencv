...
class ClassInfo(object):
        ...
        if decl:
            ...
            for m in decl[2]:
                if m.startswith("="):
                    ...
                    npos = name.rfind('.')
                    if npos >= 0:
                        self.wname = normalize_class_name(f'{name[:npos]}.{wname}')
                    else:
                        self.wname = wname
                    customname = True
                elif m == "/Map":
    ...
    def gen_def(self, codegen):
        ...
        return f'CVPY_TYPE({self.wname}, {self.name}, {self.cname if self.issimple else f"Ptr<{self.cname}>"}, {self.sname if self.issimple else "Ptr"}, {baseptr}, {constructor_name});\n'
